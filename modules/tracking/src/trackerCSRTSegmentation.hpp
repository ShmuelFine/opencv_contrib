...
public:
    int m_numBins;
    Histogram() : m_numBins(0) {}
    Histogram(int numBinsPerDimension);
    void extractForegroundHistogram(const cv::Mat& img,
            cv::Mat weights, bool useMatWeights, int x1, int y1, int x2, int y2);
    void extractBackGroundHistogram(const cv::Mat& img,
            int x1, int y1, int x2, int y2, int outer_x1, int outer_y1,
            int outer_x2, int outer_y2);
    cv::Mat backProject(cv::Mat& img);
...
{
public:
    static std::pair<cv::Mat, cv::Mat> computePosteriors(cv::Mat& img,
            int x1, int y1, int x2, int y2, cv::Mat weights, cv::Mat fgPrior,
            cv::Mat bgPrior, const Histogram &fgHistPrior, int numBins = 16);
    static std::pair<cv::Mat, cv::Mat> computePosteriors2(cv::Mat& img,
